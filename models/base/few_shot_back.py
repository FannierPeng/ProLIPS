...
class CLIP(nn.Module):
    ...
    def encode_text(self, text):
        ...
        x = token_embd + self.positional_embedding.type(self.dtype)
        x = x.permute(1, 0, 2)  # NLD -> LND
        x = self.transformer(x)
        x = x.permute(1, 0, 2)  # LND -> NLD
        x = self.ln_final(x).type(self.dtype)

        index = text.argmax(dim=-1).max()
        print(index)
        full_x = x[:, :index,:] @ self.text_projection
        # x.shape = [batch_size, n_ctx, transformer.width]
        # take features from the eot embedding (eot_token is the highest number in each sequence)
        x = x[torch.arange(x.shape[0]), text.argmax(dim=-1)] @ self.text_projection

        return x, full_x, token_embd
...
class CNN_OTAM_CLIPFSAR(CNN_FSHead):
    ...
    def __init__(self, cfg):
        ...
        with torch.no_grad():
            ...
            if isinstance(backbone,torch.nn.DataParallel):
                ...
                self.text_features_train, self.text_features_train_full, _ = back_bone.encode_text(text_templete)
            else:
                self.text_features_train, self.text_features_train_full, _ = backbone.encode_text(text_templete)

            #TODO ctx_vectors = embedding[0, 1: 1 + n_ctx, :]
            ...
            text_templete = tokenize(text_templete).cuda()
            if isinstance(backbone, torch.nn.DataParallel):
                self.text_features_test, self.text_features_test_full, _ = back_bone.encode_text(text_templete)   #[CLASS,D]
            else:
                self.text_features_test, self.text_features_test_full, _ = backbone.encode_text(text_templete)   #[CLASS,D]
    ...
    def get_feats(self, support_images, target_images, support_real_class=False, support_labels=False, backbone=backbone):
        """
        Takes in images from the support set and query video and returns CNN features.
        """
    ...
    def forward(self, inputs):
        ...
        if self.training:
            ...
            if self.args.TRAIN.TemPrompt:
                # print('support_features.shape',support_features.shape)
                # print('context_support.shape', context_support.shape)
                support_features = torch.cat([support_features.unsqueeze(2), context_support], dim=2)  #对图文拼接的特征做时序建模,support是[B,T,D], #context_support是[B, T, n_cxt, D]-->[B,T,1+n_ctx,D]
                dim = int(support_features.shape[-1])
