...
def train_epoch(
        train_loader, model, model_ema, optimizer, train_meter, cur_epoch, mixup_fn, cfg, writer=None, val_meter=None,
        val_loader=None
):
    """
    ...
    for cur_iter, task_dict in enumerate(train_loader):
                        ...
                        if hasattr(cfg.TRAIN, "USE_MOTION") and cfg.TRAIN.USE_MOTION:
                            if hasattr(cfg.TRAIN, "MOTION_COFF") and cfg.TRAIN.MOTION_COFF:
                                loss = (F.cross_entropy(model_dict["logits"], task_dict[
                                    ...
                                    task_dict["target_labels"].long()) / cfg.TRAIN.BATCH_SIZE + cfg.TRAIN.MOTION_COFF * (
                                           F.cross_entropy(model_dict["logits_motion"], task_dict["target_labels"].long()))
                            else:
        ...
        ##original_loss_back
        # if hasattr(cfg.TRAIN, "FP32") and cfg.TRAIN.FP32:
        #     loss.backward(retain_graph=False) ##original_loss_back
        # else:
...
def eval_epoch(val_loader, model, val_meter, cur_epoch, cfg, writer=None):
    ...
    for cur_iter, task_dict in enumerate(val_loader):
        ...
        if cfg.DETECTION.ENABLE:
            ...
            num_topks_correct = metrics.topks_correct(preds["move_joint"],
                                                      labels["self-supervised"]["move_joint"].reshape(
                                                          preds["move_joint"].shape[0]), (1, 5))
            top1_err, top5_err = [
...
def train_few_shot(cfg):
    ...
    assert (
                       cfg.SOLVER.MAX_EPOCH - start_epoch) % cfg.TRAIN.NUM_FOLDS == 0, "Total training epochs should be divisible by cfg.TRAIN.NUM_FOLDS."
