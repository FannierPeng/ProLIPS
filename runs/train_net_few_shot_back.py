...
def train_epoch(
    train_loader, model, model_ema, optimizer, lr_scheduler, train_meter, cur_epoch, mixup_fn, cfg, writer=None, val_meter=None, val_loader=None
):
    """
    ...
    # print("Total number of param in CLIP's image_encoder is ",
    #   sum(x.numel() for x in model.head.backbone.parameters()))
    #
    #
    # print("Total number of param in model's context layer is ",
    #   sum(x.numel() for x in model.head.context2.parameters()))
    ...
    for cur_iter, task_dict in enumerate(train_loader):
        ...
        if not hasattr(cfg.TRAIN, "FP32"):
            with autocast():
                if cfg.DETECTION.ENABLE:
                    ...
                    # model_dict = checkpoint_sequential(modules, segments, task_dict)
            ...
            if hasattr(cfg.TRAIN, "USE_CLASSIFICATION") and cfg.TRAIN.USE_CLASSIFICATION:
                if hasattr(cfg.TRAIN, "USE_CLASSIFICATION_ONLY") and cfg.TRAIN.USE_CLASSIFICATION_ONLY:
                    loss = cfg.TRAIN.USE_CLASSIFICATION_VALUE * F.cross_entropy(model_dict["class_logits"], torch.cat(
                    ...
                    if hasattr(cfg.TRAIN, "TEMPORAL_LOSS_WEIGHT") and cfg.TRAIN.TEMPORAL_LOSS_WEIGHT:
                        ...
                        loss = (F.cross_entropy(model_dict["logits"], task_dict[
                            "target_labels"].long()) + cfg.TRAIN.USE_CLASSIFICATION_VALUE * F.cross_entropy(
                            model_dict["class_logits"],
                            torch.cat([task_dict["real_support_labels"], task_dict["real_target_labels"]], 0).unsqueeze(
                                1).repeat(1, cfg.DATA.NUM_INPUT_FRAMES).reshape(-1).long())) / cfg.TRAIN.BATCH_SIZE
                        ...
                        if hasattr(cfg.TRAIN, "USE_MOTION") and cfg.TRAIN.USE_MOTION:
                            if hasattr(cfg.TRAIN, "MOTION_COFF") and cfg.TRAIN.MOTION_COFF:
                                loss = (F.cross_entropy(model_dict["logits"], task_dict[
                                    "target_labels"].long()) + cfg.TRAIN.USE_CLASSIFICATION_VALUE * F.cross_entropy(
                                    model_dict["class_logits"],
                                    torch.cat([task_dict["real_support_labels"], task_dict["real_target_labels"]],
                                              0).long())) / cfg.TRAIN.BATCH_SIZE + cfg.TRAIN.USE_CONTRASTIVE_COFF * F.cross_entropy(
                                    model_dict["logits_s2q"], task_dict[
                                        "target_labels"].long()) / cfg.TRAIN.BATCH_SIZE + cfg.TRAIN.USE_CONTRASTIVE_COFF * F.cross_entropy(
                                    model_dict["logits_q2s"], task_dict[
                                        "target_labels"].long()) / cfg.TRAIN.BATCH_SIZE + cfg.TRAIN.USE_CONTRASTIVE_COFF * F.cross_entropy(
                                    model_dict["logits_s2q_motion"], task_dict[
                                        "target_labels"].long()) / cfg.TRAIN.BATCH_SIZE + cfg.TRAIN.USE_CONTRASTIVE_COFF * F.cross_entropy(
                                    model_dict["logits_q2s_motion"], task_dict[
                                        "target_labels"].long()) / cfg.TRAIN.BATCH_SIZE + cfg.TRAIN.MOTION_COFF * (
                                           F.cross_entropy(model_dict["logits_motion"],
                                ...
                                if hasattr(cfg.TRAIN, "USE_RECONS") and cfg.TRAIN.USE_RECONS:
                                    loss = (F.cross_entropy(model_dict["logits"], task_dict[
                                        ...
                                        model_dict["logits_s2q_motion"], task_dict[
                                            "target_labels"].long()) / cfg.TRAIN.BATCH_SIZE + cfg.TRAIN.USE_CONTRASTIVE_COFF * F.cross_entropy(
                                        model_dict["logits_q2s_motion"], task_dict[
                                            "target_labels"].long()) / cfg.TRAIN.BATCH_SIZE + cfg.TRAIN.RECONS_COFF * \
                                           model_dict["loss_recons"]
                                else:
                                    loss = (F.cross_entropy(model_dict["logits"], task_dict[
                                        "target_labels"].long()) + cfg.TRAIN.USE_CLASSIFICATION_VALUE * F.cross_entropy(
                                        model_dict["class_logits"],
                                        ...
                                        model_dict["logits_s2q_motion"], task_dict[
                                            "target_labels"].long()) / cfg.TRAIN.BATCH_SIZE + cfg.TRAIN.USE_CONTRASTIVE_COFF * F.cross_entropy(
                                        model_dict["logits_q2s_motion"],
                                        task_dict["target_labels"].long()) / cfg.TRAIN.BATCH_SIZE
                        else:
                            if hasattr(cfg.TRAIN, "USE_RECONS") and cfg.TRAIN.USE_RECONS:
                                loss = (F.cross_entropy(model_dict["logits"], task_dict[
                                    "target_labels"].long()) + cfg.TRAIN.USE_CLASSIFICATION_VALUE * F.cross_entropy(
                                    ...
                                    model_dict["logits_s2q"], task_dict[
                                        "target_labels"].long()) / cfg.TRAIN.BATCH_SIZE + cfg.TRAIN.USE_CONTRASTIVE_COFF * F.cross_entropy(
                                    model_dict["logits_q2s"],
                                    task_dict["target_labels"].long()) / cfg.TRAIN.BATCH_SIZE + cfg.TRAIN.RECONS_COFF * \
                                       model_dict["loss_recons"]
                            else:
                                loss = (F.cross_entropy(model_dict["logits"], task_dict[
                                    "target_labels"].long()) + cfg.TRAIN.USE_CLASSIFICATION_VALUE * F.cross_entropy(
                                    ...
                                    model_dict["logits_s2q"], task_dict[
                                        "target_labels"].long()) / cfg.TRAIN.BATCH_SIZE + cfg.TRAIN.USE_CONTRASTIVE_COFF * F.cross_entropy(
                                    model_dict["logits_q2s"], task_dict["target_labels"].long()) / cfg.TRAIN.BATCH_SIZE
                    else:  ##Here, 用的这个loss
                        loss = (F.cross_entropy(model_dict["logits"], task_dict[
                            "target_labels"].long()) + cfg.TRAIN.USE_CLASSIFICATION_VALUE * F.cross_entropy(
                            model_dict["class_logits"],
        ...
        if math.isnan(loss):
            ...
            ##original_loss_back
            if hasattr(cfg.TRAIN, "FP32") and cfg.TRAIN.FP32:
                ...
                ##scaler_loss_back
                optimizer.zero_grad()
                scaler.scale(loss).backward()  # scaler.scale(loss).backward(retain_graph=False)
                continue
...
def train_few_shot(cfg):
    ...
    train_epoch(
        train_loader, model, model_ema, optimizer, lr_scheduler, train_meter, cur_epoch, mixup_fn, cfg, writer, val_meter, val_loader
    )
